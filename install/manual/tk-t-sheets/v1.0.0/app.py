...
class t_sheets_connect(Application):
    ...
    def init_app(self):
        ...
        time_events = threading.Thread(name='timeEvents', target=self.timed_events)
        time_events.setDaemon(True)
        time_events.start()
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Watch folders and timer events
    # ------------------------------------------------------------------------------------------------------------------
    def timed_events(self):
        # Figure out a way to have small watch events working in the background
        # Needs to check for mouse inactivity between 12 pm and 2:30 pm for lunch triggers
        # But, it also needs to watch for general inactivity.  This could also be blended with the previous, so
        # if there is inactivity and it's between the hours of 12 and 2:30, then it would suggest a lunch break.
        # It will also need to cycle in on itself and re-run itself if the condition is no longer true.
        print 'timed event triggered.'

    def overtime_check(self):
        # Routine to check the status of a users daily overtime
        pass
        ...
        while self._generator:
            if s < 59:
                ...
                if m < 59:
                    ...
                    m = 0
                    s = 0
            # This is where the trigger codes go.
            yield
